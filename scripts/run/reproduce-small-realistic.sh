...
while read -r line; do
    ...
    PATH=build:$PATH RDMAV_FORK_SAFE=1 nohup mpirun -np 32 -map-by numa:PE=1 -bind-to core build/mallob -t=1 -q -c=1 -ajpc=384 -ljpc=4 -T=600 -log=logs/realistic-$rno -v=4 -warmup -satsolver=kclkclcl -pls=0 -sjd=1 -job-template=instances/job-template-priorities.json -job-desc-template=instances/selection_isc2020_394.txt -client-template=instances/$clienttemplate
    
    echo "Experiment done"
    echo "******************************************************"
    echo ""

done < scripts/run/configs-small-realistic.csv
